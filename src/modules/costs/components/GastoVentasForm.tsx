...
export default function GastoVentasForm({ control, register }: Props) {
  const setTotalGastosVentas = useCostStore(
    (state) => state.setTotalGastosVentas
  );
  const totalGastosVentas = useCostStore((state) => state.totalGastosVentas);
  const { fields, append, remove } = useFieldArray({
    control,
    name: "gastosVentas",
  });
  const gastosVentas = useWatch({
    control,
    name: "gastosVentas",
  });

  useEffect(() => {
    if (!gastosVentas) return;

    const total = gastosVentas.reduce((acc, item) => {
      const { valorTotal } = item;
      return acc + valorTotal;
    }, 0);

    setTotalGastosVentas(total);

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [JSON.stringify(gastosVentas)]);
  return (
    <>
          ...
          >
            <div>
              ...
              <input
                ...
                {...register(`gastosVentas.${index}.nombre`, {
                  required: true,
                })}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                {...register(`gastosVentas.${index}.valorTotal`, {
                  valueAsNumber: true,
                })}
              />
            </div>
      ...
      <div className="flex justify-end mt-6">
        <span className="inline-block bg-blue-100 text-blue-800 text-sm md:text-base font-semibold px-5 py-2 rounded-md shadow-sm border border-blue-300">
          ...
          <span className="font-bold">
            {formattCurrency(totalGastosVentas)}
          </span>
        </span>
      </div>
    </>
