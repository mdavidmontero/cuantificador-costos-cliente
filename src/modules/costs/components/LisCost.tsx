import { TrashIcon, EyeIcon } from "lucide-react";
import type { SchemaCost } from "../schemas";
import { formattCurrency, formattDate } from "@/lib";
import { useNavigate } from "react-router-dom";

interface ListCostProps {
  data: SchemaCost[];
}

export default function ListCost({ data }: ListCostProps) {
  const navigate = useNavigate();

  return (
    <div className="w-full overflow-x-auto rounded border border-gray-200 dark:border-gray-700 shadow-sm">
      <table className="min-w-full text-sm text-left text-gray-700 dark:text-gray-300">
        <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0 z-10">
          <tr className="text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-300">
            <th className="px-4 py-3 whitespace-nowrap">Fecha</th>
            <th className="px-4 py-3 whitespace-nowrap">Producto</th>
            <th className="px-4 py-3 whitespace-nowrap">Cantidad</th>
            <th className="px-4 py-3 whitespace-nowrap">Cost. Operación</th>
            <th className="px-4 py-3 whitespace-nowrap">Gast. Mercadeo</th>
            <th className="px-4 py-3 whitespace-nowrap">Gast. Producción</th>
            <th className="px-4 py-3 whitespace-nowrap">Costo Unitario</th>
            <th className="px-4 py-3 whitespace-nowrap">Precio Venta</th>
            <th className="px-4 py-3 whitespace-nowrap">% Utilidad</th>
            <th className="px-4 py-3 whitespace-nowrap text-center">
              Acciones
            </th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
          {data.map((registro) => {
            const costo = registro.costoProduccion;
            return (
              <tr
                key={registro.id}
                className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
              >
                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-200 whitespace-nowrap">
                  {formattDate(registro.date?.toString() ?? "")}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {registro.producto?.nombre || "—"}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {registro.cantidadProducida ?? "—"}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {costo?.totalCostosOperacion != null
                    ? formattCurrency(costo.totalCostosOperacion)
                    : "—"}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {costo?.totalGastosMercadeo != null
                    ? formattCurrency(costo.totalGastosMercadeo)
                    : "—"}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {costo?.totalGastosProduccion != null
                    ? formattCurrency(costo.totalGastosProduccion)
                    : "—"}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {costo?.totalCostoProduccionUnitario != null
                    ? formattCurrency(costo.totalCostoProduccionUnitario)
                    : "—"}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {costo?.precioVentaUnitario != null
                    ? formattCurrency(costo.precioVentaUnitario)
                    : "—"}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {costo?.margenUtilidadUnitario != null
                    ? `${costo.margenUtilidadUnitario.toFixed(2)}%`
                    : "—"}
                </td>
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <div className="flex items-center justify-center gap-2">
                    <button
                      onClick={() => navigate(`/costos/${registro.id}`)}
                      className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-md transition"
                    >
                      <EyeIcon className="w-4 h-4" />
                      Ver
                    </button>
                    <button
                      onClick={() => {}}
                      className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-md transition"
                    >
                      <TrashIcon className="w-4 h-4" />
                      Eliminar
                    </button>
                  </div>
                </td>
              </tr>
            );
          })}
        </tbody>
      </table>
    </div>
  );
}
